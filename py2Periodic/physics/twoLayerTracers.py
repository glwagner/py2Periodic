...
class model(doublyPeriodicModel):
    ...
    def visualize_model_state(self):
        ...
        Ro = 2e-1
        qmin, qmax = -Ro, Ro
        cmin, cmax = 1e-2*self.c1.max(), self.c1.max()
        xkm, ykm = self.x*1e-3, self.y*1e-3
        ...
        quadMesh = axArr[0, 0].pcolormesh(xkm, ykm, self.q1/self.f0, 
            vmin=qmin, vmax=qmax, cmap='RdBu_r')
            
        axArr[0, 1].pcolormesh(xkm, ykm, self.q2/(self.f0*self.delta), 
            vmin=qmin, vmax=qmax, cmap='RdBu_r')

        axArr[1, 0].pcolormesh(xkm, ykm, self.c1, vmin=cmin, vmax=cmax)
        axArr[1, 1].pcolormesh(xkm, ykm, self.c2/self.delta, vmin=cmin, vmax=cmax)
