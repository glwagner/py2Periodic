...
class model(doublyPeriodicModel):
    ...
    def _calc_right_hand_side(self, soln, t):
        ...
        k2 = self._k2
        l2 = self._l2
        oneOverKsq = self._oneOverKsq
        invE = self._invE

        self.q = np.real(self.ifft2(qh))

        # Derivatives of A in physical space
        ne.evaluate("jk*Ah", out=UAh)
        self.ifft2(UAh, out=Ax)

        ne.evaluate("jl*Ah", out=UAh)
        self.ifft2(UAh, out=Ay)

        ne.evaluate("-k2*Ah", out=UAh)
        self.ifft2(UAh, out=Axx)

        ne.evaluate("-l2*Ah", out=UAh)
        self.ifft2(UAh, out=Ayy)

        ne.evaluate("-kl*Ah", out=UAh)
        self.ifft2(UAh, out=Axy)

        ne.evaluate(
            "-alpha/2.0*(k2 + l2 + (4 + 3*alpha)*kappa**2.0)*Ah",
            out=UAh)
        self.ifft2(UAh, out=EA)


        # Calculate streamfunction
        ne.evaluate("-qh*oneOverKsq", out=psih)

        # Mean velocities
        ne.evaluate("-jl*psih", out=UAh)
        U = np.real(self.ifft2(UAh))

        ne.evaluate("jk*psih", out=UAh)
        V = np.real(self.ifft2(UAh))


        # Right hand side for A, in steps:
        ## 1. Advection term, 
        ne.evaluate("U*EA", out=UAh)
        UAh = self.fft2(UAh)
        ne.evaluate("-jk*UAh", out=RHS1)

        ne.evaluate("V*EA", out=UAh)
        UAh = self.fft2(UAh)
        ne.evaluate("invE*(RHS1 - jl*UAh)", out=RHS1)
            

        ## 2. Refraction term
        ne.evaluate("q*(1j*sigma*Ax - f0*Ay)", out=UAh)
        UAh = self.fft2(UAh)
        ne.evaluate("jk*UAh", out=RHS0)

        ne.evaluate("q*(1j*sigma*Ay + f0*Ax)", out=UAh)
        UAh = self.fft2(UAh)
        ne.evaluate("RHS1 - invE/f0*(RHS0 + jl*UAh)", out=RHS1)


        ## 3. 'Middling' difference Jacobian term.
        ne.evaluate(
            "V*(1j*sigma*Axy - f0*Ayy) - U*(1j*sigma*Ayy + f0*Axy)", 
            out=UAh)
        UAh = self.fft2(UAh)
        ne.evaluate("jk*UAh", out=RHS0)

        ne.evaluate(
            "U*(1j*sigma*Axy + f0*Axx) - V*(1j*sigma*Axx - f0*Axy)", 
            out=UAh)
        UAh = self.fft2(UAh)
        ne.evaluate(
            "RHS1 + invE*(2j*sigma/f0**2.0)*(RHS0 + jl*UAh)", 
            out=RHS1)


        # Two-dim turbulence
        ne.evaluate("U*q", out=UAh)
        UAh = self.fft2(UAh)
        ne.evaluate("-jk*UAh", out=RHS0)

        ne.evaluate("V*q", out=UAh)
        UAh = self.fft2(UAh)
        ne.evaluate("RHS0 - jl*UAh", out=RHS0)
    ...
    def _init_problem_parameters(self):
        ...
        self._oneOverKsq = 1.0 / self._divSafeKsq
        ...
        self.UAh = np.zeros(self.specVarShape, np.dtype('complex128'))
