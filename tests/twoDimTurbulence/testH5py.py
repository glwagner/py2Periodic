...
file = h5py.File('twoDimTurbSnapshots.hdf5', 'w')
snapshots = file.create_dataset("vorticity", 
    (turb.nx, turb.nx, nSnapshots), dtype='c128')

# Step the model forward in time
for i in xrange(nSnapshots):
    turb.step_nSteps(nSteps=1e2, dnLog=1e2)
